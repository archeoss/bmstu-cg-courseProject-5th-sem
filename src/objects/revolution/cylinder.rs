...
impl Cylinder {
    ...
    pub fn subdiv(&mut self, subdivision: usize, lower_radius: f64, upper_radius: f64) {
...
//
impl From<Rc<RefCell<FrameModel>>> for Cylinder {
    fn from(frame_model: Rc<RefCell<FrameModel>>) -> Self {
        ...
        Self {
            frame_model,
            upper_radius: maxr.x() - minr.x(),
            lower_radius: maxr.x() - minr.x(),
...
impl From<Cylinder> for Rc<RefCell<FrameModel>> {
    fn from(sphere: Cylinder) -> Self {
